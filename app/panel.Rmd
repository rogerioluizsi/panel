...
load ("data/uf_years.Rdata")
load ("data/mun_years.Rdata")
load ("data/meso_years.Rdata")

load ("data/Sgeo_notas.Rdata")
load ("data/geo_notas.Rdata")

load("data/multi_year_outputs.Rdata")
load("data/fixed_effects_outputs.Rdata")
...
----------------------------------------------------------------------
**Todos os gráfico são interativos**. 

O Painel do Ensino Médio Brasileiro utliza os dados do ENEM e do Censo Escolar dos anos de 2009 a 2019. 
...
Além das análises descritivas, o painel traz análises utilizando de Mineração de Dados para auxiliar no entendimento dos determinantes do desempenho escolar ao longo dos anos.

Autores: [Rogerio Luiz](https://github.com/rogerioluizsi), [Paulo Adeodato](https://lemanncenter.stanford.edu/people/paulo-adeodato) e [Kellyton Brito](https://https://kellyton.com.br/)
...
Após clicar em plotar, não é possível cancelar a ação. Devido a grande quantidade de dados e às limitações do R Shiny, os gráficos dessa aba demoram cerca de **60s** para carregarem. Aguarde, valerá a pena :).
...
Column 
----------------------------------------------------------------------
### Panorama desempenho x nº inscritos X tipo de escola - Local (Esse gráfico não é responsivo (limitação ggigraph), ajuste o tamanho da tela para ver os 3 tipos de escolas)
...
Column 
------------------------------------------
### Panorama do desempenho x nº inscsritos X tipo de escola - Nacional (Esse gráfico não é responsivo (limitação ggigraph), ajuste o tamanho da tela para ver os 3 tipos de escolas) {.nacional}
...
**Resumo:** Em uma abordagem de Mineração de Dados, baseado no CRISP-DM, para cada ano no peírodo 2009-2019, os dados do Enem e do Censo escolar foram padronizadoe e, no grão escola, submetidos a modelos de classificaçãi a fim de descrever uma escola boa ou ruim.
...
Os gŕaficos são interativos e é possível baixar ou consultar a tabela com os resultados dos modelos detalhados para cada ano.
...
theme(axis.text.x = element_text(angle = 45, hjust =1))+xlab(element_blank())+ylab("Importãncia das variáveis")+ 
...
histo<- plot_multyi_years()%>%ungroup()
#glimpse(histo)
...
histo2<- plot_multyi_years()%>%ungroup()
#glimpse(histo)
...
ggplot(track_importance, aes(x=as.character(year), group=grupo, fill=grupo)) +
       geom_density(adjust=0.5,position="fill") +xlab(element_blank())+ylab("Fração")     
})
```
...
**Resumo:** Nesta análise, usando a mesma base de dados da Anaçise I, o comportamento longitudinal dos dados foram considerados. Um modelo de efeitos-fixos sob regressão logística foi implementado para 10 diferentes períodos em uma configuração de validação cruzada. O primeiro período tem início de 2009 até 2019 e apenas as escolas que se repetem em todos os anos do período foram consideradas. Os outros períodos tem o ínicio variando de 2010 a 2018 sempre terminando em 2019. Dessa forma o número de escolas presentes em cada período varia de forma crescente.

Neste experimento é possível detectar de maneira causal mudanças que estejam relacionadas à evolução das notas das escolas ao londo do período analisado. Para tal, devemos assumir que as variavéis que impactam a qualidade das escolas e que não estão presentes nas bases de dados são oriundas principalmente da política individual de cada escola e de cada estado. Além disso, devemos assumir que qualquer que seja essa influência, ela é contante para uma determinada escola e para um determinado estado no período analisado. 

Quanto maior número de escolas consideradas maior a confiabilidade dos resultados.
...
causal_plot<- reactive( 
fig51<- result_models_fixed_effects%>%filter(tp_escola == input$dep_adm4)
)
...
theme(axis.text.x = element_text(angle = 45, hjust =1))+xlab(element_blank())+ylab("Beta Coefficients")+ 
  theme(legend.position="right", 
        legend.title = element_blank())+ 
  scale_x_discrete(labels=labels) + 
...
Baseado em artigos publicados com essas mesmas bases de dados, construímos um painel que reproduz a metodologia dessas pesquisas e condensa os principais resultados para o período de 2009 a 2019.


**Os principais objetivos do Painel são**: a) analisar as principais dimensões presentes nas discussões da educação nacional e que são observáveis nas bases de dados e b) observar o comportamento dos determinantes do ensino médioao ao longo dos anos e c) detectar intervenções relacionadas à evolução da qualidade das escolas de ensino médio .

Espera-se que este painel possa servir de subsídio para especialistas da educação em suas pesquisas e análises. Além disso, comentários e críticas de colegas que trabalham com Educação,  Mineração de Dados e Suporte a Decisão são muito bem-vindos.

**Texto MPO1'ono Medium**: [Painel Interativo - Uma década do Ensino Médio brasileiro](https://medium.com/@rogerio.luiz/painel-interativo-uma-d%C3%A9cada-do-ensino-m%C3%A9dio-brasileiro-1c077f260349)
