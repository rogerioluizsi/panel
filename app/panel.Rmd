...
uf_years<-read_feather ("~/projects/panel/data/uf_years.feather")
mun_years<-read_feather ("~/projects/panel/data/mun_years.feather")
meso_years<- read_feather ("~/projects/panel/data/meso_years.feather")

Sgeo_notas<-read_feather ("~/projects/panel/data/Sgeo_notas.feather")
geo_notas<-read_feather ("~/projects/panel/data/geo_notas.feather")

result_models<-read_feather ("~/projects/panel/data/result_models.feather")
ia_group_analysis<-read_feather ("~/projects/panel/data/ia_group_analysis.feather")
...
output$fig13<-renderPlotly({ 
  withProgress(message = 'Plotando gŕaficos Nacionais',
                 detail = 'Aguarde...', value = 0, {
      for (i in 1:15) {
        incProgress(1/15)
      }
...
**Como interpretar os gráficos:** as linhas são a distribuição ao longo dos anos do grau de influência da variável "X", que está entre as 6 principais variáveis para cada ano ecom níve de significância menor que 0,05. Os Valores positivos são bons descritores de boas escolas ao contrário dos valores negativos. 
...
Inputs {.sidebar data-width=350}
----------------------------------------------------------------------
**Resumo:** A variáveis do Censo Escolar e do ENEM foram divividas em 3 grupos. a) infraestrutura b) socioeconômico e c) educação dos pais e professores (hipótese). Para cada ano, os grupo de variáveis teve seu poder de classificação avaliado pela Área sob a curva ROC (AUC). Foi utilizado a técnica de regressão logística em uma abordaem de validação cruzada. A união de todos os grupos também foi avaliada (FULL). O estudo visa indicar como principal alternativa de melhoria da educação básica,  o investimento na educação dos pais e profesores, visto a complexidade e limitação das políticas de transferência de renda e na simplicidade dos esforços na melhoria da infraestrutura das escolas.
...
Inputs {.sidebar data-width=200}
----------------------------------------------------------------------
A influencia da educação dos pais no desempenho dos alunos pode ser apenas uma extensão às variáveis socioeconômicas - conhecidos determinantes do desempenho escolar. Afinal, pais com maior escolaridade tendem possuir melhores empregos, melhores salários e buscam melhores escolas.

Observe a influência da educação dos pais controlando pela infraestutrura escolar e pela renda per capita.
```{r}
...
fig1<-plot_ly(trace1, x= ~EDU_MAE, y= ~prop, 
  ...
  add_lines(x=trace1$EDU_MAE, y = 0.25, line =list(width = 0.2, dash = 'dash', color = "black"))
...
```{r}
output$fig79<- renderPlotly({
sub<-Sgeo_notas_nacional%>%filter(CO_DEPENDENCIA_ADM=='Municipal+Estadual')%>%
  filter(RENDA_PERCAPITA >= input$range[1] & RENDA_PERCAPITA <= input$range[2])%>%filter(get(input$INFRA) ==1)
...
plot_ly(trace1, x= ~EDU_MAE, y= ~prop, 
  ...
  add_trace(x =trace2$EDU_PAI, y= trace2$prop, 
            ...
            text = ~paste('</b>Educação do Pai:</b> 
                          ...
                          '</br> Quantidade de alunos: ', trace2$quantidade))%>%
  layout(yaxis = list(title = 'Concentraçao de bons alunos'), xaxis = list(title =""), title='Proporção de bons alunos pela ecolaridade dos pais')
...
output$fig80<- renderPlotly({
sub<-Sgeo_notas_nacional%>%filter(CO_DEPENDENCIA_ADM=='Privada')%>%
  filter(RENDA_PERCAPITA >= input$range[1] & RENDA_PERCAPITA <= input$range[2])%>%filter(get(input$INFRA) ==1)
...
output$fig81<- renderPlotly({
sub<-Sgeo_notas_nacional%>%filter(CO_DEPENDENCIA_ADM=='Federal')%>%
  filter(RENDA_PERCAPITA >= input$range[1] & RENDA_PERCAPITA <= input$range[2])%>%filter(get(input$INFRA) ==1)
...
Baseado em artigos publicados pelo Paulo Adeodato e por mim com essas mesmas bases de dados, construímos um painel que reproduz a metodologia dessas pesquisas e condensa os principais resultados para o período decenal de 2009 a 2018.

"Embora recentemente (jun/2020)os dados do ENEM 2019 tenham sido publicados pelo INEP, eles não foram incorporados ao Painel, pois todas as análises consideram também os dados do Censo Escolar que ainda não foram publicados para este mesmo ano."
...
Texto no medium: [Painel Interativo - Uma década do Ensino Médio brasileiro](https://medium.com/p/1c077f260349/edit)
...
renderTable({
    data.frame(Variable = names(sessioninfo()),
